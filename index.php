         ...
         switch ($_POST['jsaction'])
         {
            ...
            case "withdraw":
               ...
               if (!WITHDRAWALS_ENABLED) {
                  ...
                  $json['message'] = "O valor do saque excede seu saldo...";
               } else {
         ...
         switch ($_POST['action'])
         {
            ...
            case "withdraw":
               if (!WITHDRAWALS_ENABLED) {
                  ...
                  $error['type'] = "withdraw";
                  $error['message'] = "Erro no token.";
                  $_SESSION['token'] = sha1('@s%a$l£t#'.rand(0,10000));
               } elseif ($_POST['amount'] > $balance) {
                  $error['type'] = "withdraw";
                  $error['message'] = "O valor do saque excede seu saldo.";
               } else {
                  $withdraw_message = $client->withdraw($user_session, $_POST['address'], (float)$_POST['amount']);
                  $_SESSION['token'] = sha1('@s%a$l£t#'.rand(0,10000));
                  header("Location: index.php");
            ...
            case "logout":
               ...
               break;
	    case "support":
	 $error['message'] = "Por favor, fale com nosso suporte via email: $support";
                echo "ID do Suporte: ";
                     ...
                     switch ($_POST['jsaction'])
                     {
                        ...
                        case "withdraw":
                           ...
                           if (!WITHDRAWALS_ENABLED) {
                              ...
                              $json['message'] = "O valor do saque excede o seu saldo.";
                           } else {
                     ...
                     switch ($_POST['action'])
                     {
                        ...
                        case "password":
                           if ((is_numeric($_GET['i'])) && (!empty($_POST['password'])))
                           {
                              ...
                              {
                                 $error['type'] = "password";
                                 $error['message'] = "Senha alterada com sucesso!";
                                 header("Location: index.php?a=info&i=" . $info['id']);
                              } else {
                                 $error['type'] = "password";
                                 $error['message'] = $result;
